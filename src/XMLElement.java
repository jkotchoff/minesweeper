...
//THE FOLLOWING LINE IS COMMENTED SO THAT I CAN INSTANTIATE THIS CLASS DIRECTLY
//FROM THE SAME DIRECTORY WITH NO PACKAGE CONFIGURATION
//NOTE: I also added a method titled "selectSingleNode()"
//- Jason Kotchoff 10/01/2002
//package nanoxml;
...
public class XMLElement
{
   ...
   /**
    * I added this - Jason Kotchoff
    * it's pretty hacked because Internet Explorer doesnt seem
    * to recognise Vector.lastElement()
    * 13/01/2002
    */
   public void removeLastChild() {
      this.children.removeElementAt(children.size()-1);
   }

   /**
    * Splits @param str using @separator as a tokeniser and
    * returns an array of strings. Similar to StringTokenizer
    * however this method allows direct access to a particular
    * token without enumeration using the syntax:
    * split("test/something")[1]
    */
   public String[] split (String str, String separator) {
      ...
      String temp_str = str;
      int num_separators = contains(str, separator);

      String return_array[] = new String[num_separators+1];

      for (int i = 0, separator_index; i <= num_separators; i++) {
         separator_index = temp_str.indexOf(separator);
         if (i < num_separators) {
            return_array[i] = temp_str.substring(0, separator_index);
            temp_str = temp_str.substring(separator_index+1, temp_str.length());
         }
         else {
   ...
   /**
    * Returns the number of occurences of str withing search_str
    */
   public int contains(String search_str, String str) {
      int occurences = 0;
      String temp_str = search_str;
      while (temp_str.indexOf(str) != -1) {
         occurences++;
         temp_str = temp_str.substring(temp_str.indexOf(str)+1, temp_str.length());
      }
      return occurences;
   }

   /**
    * I added this however it only supports basic functionality (node paths
    * only, no attributes or *'s in the path or anything) - Jason Kotchoff
    * 10/01/2002
    */
   public XMLElement selectSingleNode(String node_path) {
      if (node_path.indexOf("/") == -1) {
         return (node_path.compareTo(getName().trim()) == 0) ? this:null;	//note: JKTrim() is added because of my flawed PHP XML parser on the backend
      }

      XMLElement child_node = null;
      String child_path, child_node_name;

      child_path = node_path.substring(node_path.indexOf("/")+1, node_path.length());

      if (child_path.indexOf("/") == -1)
         child_node_name = child_path;
      else
         child_node_name = split(child_path, "/")[0];

      Enumeration enum = enumerateChildren();
      while (enum.hasMoreElements() && child_node == null) {
          XMLElement child = (XMLElement) enum.nextElement();
          if (child.getName().compareTo(child_node_name) == 0) {
             child_node = child;
          }
      }
      ...
      return child_node.selectSingleNode(child_path);
   }
